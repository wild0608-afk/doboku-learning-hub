...
100問追加完了。2026-05-24 時点の実績：

```txt
Set 1：TK-2026-0001〜0025  宅建業法 25問        → 完了
Set 2：TK-2026-0026〜0050  権利関係 25問         → 完了
Set 3：TK-2026-0051〜0075  法令上の制限 25問     → 完了
Set 4：TK-2026-0076〜0100  税・その他 25問       → 完了
```
...
宅建業法：    27問（既存2問 + 新規25問）
権利関係：    27問（既存2問 + 新規25問）
法令上の制限：27問（既存2問 + 新規25問）
税・その他：  27問（既存2問 + 新規25問）
```

新規100問はすべて `verified: false` / `confidence: "unchecked"` / `generatedBy: "Gemini"`。
公式確認は Phase 5 で実施予定。
...
### 2026-05-24 付箋ID処理の修正（script.js）

- 修正前：`const qid = parseInt(el.dataset.qid);`
- 修正後：`const qid = el.dataset.qid;`
- 理由：`parseInt("TK-2026-0001")` は `NaN` になるため、付箋の保存・復習・状態表示が正常に動作しなかった。文字列IDに対応するため `parseInt` を除去した。
